...
export default function ResiForm({ onSubmit, submitting, onSuccess }: ResiFormProps) {
  ...
  const inputClass = (field: string) =>
    `w-full bg-slate-50 border rounded-lg px-3 py-2 text-sm text-slate-700 focus:outline-none focus:border-blue-500 transition-colors placeholder:text-slate-300 ${
      errors[field] ? 'border-rose-400' : 'border-[#E2E8F0]'
    }`;
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] p-5 space-y-4 shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-slate-700">Tambah Resi Baru</h3>
      </div>
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-7 gap-3">
        ...
        <div className="space-y-1">
          ...
          />
          {errors.noResi && <p className="text-[10px] text-rose-500">{errors.noResi}</p>}
        </div>
        <div className="space-y-1">
          <input
            type="text"
          ...
          />
          {errors.agen && <p className="text-[10px] text-rose-500">{errors.agen}</p>}
        </div>
        <select
          value={layanan}
          onChange={(e) => setLayanan(e.target.value)}
          className="w-full bg-slate-50 border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm text-slate-700 focus:outline-none focus:border-blue-500 transition-colors cursor-pointer font-medium"
        >
        ...
        <div className="space-y-1">
          ...
          />
          {errors.petugas && <p className="text-[10px] text-rose-500">{errors.petugas}</p>}
        </div>
        <select
          value={statusResi}
        ...
        <button
          type="submit"
          disabled={submitting}
          className="bg-[#1E293B] hover:bg-slate-700 text-white font-medium text-sm rounded-lg px-4 py-2 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
